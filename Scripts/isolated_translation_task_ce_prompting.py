"""
Description:
This script generates three English translations for each Chinese source sentence
...
English translations based on a single example.
...
for i, d in enumerate(data):
    if not d["TT1"]["text"] and not d["TT2"]["text"] and not d["TT3"]["text"]:
        st = d["ST"]["zh"]
        prompt = f"Translate the following sentence into English. Here is an example: ST: 我喜欢猫。TT: I love cats. Now translate the following. ST: {st}"
        valid_tts = []
        ...
        with open("path/to/translations-with-ref-ce.json", "w", encoding="utf-8") as f:
